...
<head>
    <meta charset="utf-8">
    <title>Hoja de Ruta - {{ $correspondencia->codigo }}</title>
    <style>
        @page {
            margin: 10mm;
        }
    ...
    <div class="hoja-de-ruta-container">
        <table width="100%" style="margin-bottom: 20px; border-bottom: 2px solid #000;">
            <tr>
                ...
                <td width="8%" style="text-align: left; center-align: top;">
                    <img src="{{ public_path('images/logo_upea.png') }}" alt="Logo UPEA" style="height: 60px;"><br>
                ...
                <td width="40%" style="text-align: center; vertical-align: top;">
                    ...
                    <span style="font-size: 14px; font-weight: bold;">{{ $department_name }}</span>
                </td>

                <!-- Columna 3: Número y Gestión -->
                <td width="20%" style="text-align: right; vertical-align: top;">
                    <span style="font-size: 10px;">N° HOJA DE RUTA: {{ $correspondencia->rut }}</span><br>
                    <span style="font-size: 10px;">GESTIÓN:
                        {{ $correspondencia->anio ?? \Carbon\Carbon::parse($correspondencia->fecha)->year }}</span>
                </td>
            </tr>
        </table>
                ...
                <tbody>
                    <tr>
                        <td class="main-info-field">
                            ...
                            <span>{{ $correspondencia->fecha->format('d/m/Y') }}</span>
                        </td>
                        <td class="main-info-field">
                            <label>Hora de recepción.:</label>
                            <span>{{ $correspondencia->hora->format('H:i') }}</span>
                        </td>
                        <td class="main-info-field">
                            <label>Código de seguimiento:</label>
                            <span>{{ $correspondencia->codigo }}</span>
                        </td>
                    </tr>
                    <tr>
                        <td class="main-info-field">
                            <label>N° de Files:</label>
                            <span>{{ $correspondencia->folder }}</span>
                        </td>
                        <td class="main-info-field">
                            <label>N° de Fojas:</label>
                            <span>{{ $correspondencia->fojas }}</span>
                        </td>
                        <td class="main-info-field">
                            <label>Teléfono:</label>
                            <span>{{ $correspondencia->fono ?? '' }}</span>
                        </td>
                    </tr>
                    <tr>
                        <td class="main-info-field" colspan="2"> {{-- Ocupa las 3 celdas --}}
                            <label>Remitente:</label>
                            <span>{{ $correspondencia->remitente }}</span>
                        </td>
                        <td class="main-info-field" colspan="2"> {{-- Celda separada para compactar --}}
                            <label>Lugar de origen:</label>
                            <span>{{ $correspondencia->unidad ?? '' }}</span>
                        </td>
                    </tr>
                   

                    <tr>
                        <td class="main-info-field" colspan="3"> {{-- Ocupa las 3 celdas --}}
                            <label>Referencia:</label>
                            <span>{{ $correspondencia->referencia }}</span>
                        </td>
                    </tr>
                </tbody>
            </table>
        ...
        @for ($i = 0; $i < 3; $i++)
            <section class="destination-block">
                <div class="destination-header">DESTINO N° {{ $i + 1 }}</div>
                <table class="instruction-table">
                    <tr>
                        <td class="instructions-column-content">
                            <span class="instruction-header">INSTRUCCIÓN Nº {{ $i + 1 }}</span>
